 ...
 *   sentence to show someone. The form's own floor should make this rare.
...
export function authMessage(error: unknown): string {
  ...
  if (error instanceof ApiError) {
    switch (error.status) {
      ...
      case 422:
        return 'Check the email address, and make sure the password is at least 12 characters.';
      case 429:
