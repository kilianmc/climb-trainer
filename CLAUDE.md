...
- 2026-09-04 — halving a single `(phase, aspect)` candidate pool leaves `_DISTINCT_SHARE_FLOOR_PCT` in `tests/test_planner_library_reach.py` green, because most rows appear in several phases: the floor bites on a rotation defect or a library-wide loss, never on one cell, and nothing else covers that shape. This PR moved three rows between pools, which is exactly it.
