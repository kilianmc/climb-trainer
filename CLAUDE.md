...
- Climbing is allocated first and the week has a floor; the band is a target range, not only a floor; eligibility is `prescribable()` and on-the-wall is a preference, never a filter → *The plan generator*
...
- 2026-09-04 — authored `instructions` now carry a SUPERLATIVE about the library's own contents (`explosive_move_intervals`: "the cheapest on-the-wall power work in the library in minutes"). It was measured true when written and no guard covers it, so authoring one cheaper on-wall `power` row makes the copy lie silently. Found by re-measuring the claim during PR C: unqualified it was already false, `standing_jumps` costs 3.4 min at taper against 5.4.
